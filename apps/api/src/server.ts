...
async function refreshCandidateLifecycle(){try{await db.query("update candidate_profiles set status='VENCIDO',updated_at=now() where status='ACTIVO' and valid_until<current_date");await db.query(`insert into candidate_notifications(candidate_id,type,title,message) select candidate_id,'PROFILE_EXPIRING','Tu perfil está por vencer','Tu perfil de Empleos.pa vence en 7 días. Si sigues buscando empleo, entra a tu cuenta y toca “Sigo buscando empleo · Renovar”.' from candidate_profiles where status='ACTIVO' and valid_until between current_date and current_date+7 on conflict do nothing`);await db.query("delete from auth_sessions where expires_at<now()-interval '30 days' or revoked_at<now()-interval '30 days'");await db.query("delete from password_reset_tokens where expires_at<now()-interval '7 days' or used_at<now()-interval '7 days'");}catch(error){app.log.error(error,"candidate lifecycle refresh failed");}}
